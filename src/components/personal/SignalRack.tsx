...
function Sparkline({ values }: { values: number[] }) {
  ...
  const xStep = values.length > 1 ? (width - 12) / (values.length - 1) : 0;
  const yFor = (value: number) => {
    const clamped = Math.max(0, Math.min(value, maxValue));
    return height - 8 - (clamped / maxValue) * (height - 16);
  };
  const path = values
    .map((value, index) => `${index === 0 ? "M" : "L"}${6 + xStep * index},${yFor(value)}`)
    .join(" ");
...
export default function SignalRack({ gauges, chips }: Props) {
  ...
  const clampValue = useMemo(() => (value: number) => Math.max(0, Math.min(value, 1)), []);
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-end gap-4 overflow-hidden rounded-2xl border border-border/60 bg-surface/80 px-4 pb-4 pt-6">
          {gauges.map((gauge, index) => {
            const barHeight = `${clampValue(gauge.value) * 100}%`;
            return (
